...
const CategoryCards = () => {
  ...
  const { cards, loading, error } = useCards(categoryId);
  const filteredCards = cards.filter(card => card.type === categoryId);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-white">Загрузка карт...</div>
      </div>
    );
  }
  ...
  const handleSellCard = (cardId: string, price: number) => {
    // Логика продажи будет через API
    toast({
      title: "Карта продана!",
  ...
  const handleGiftCard = (cardId: string, playerName: string) => {
    // Логика дарения будет через API
    toast({
